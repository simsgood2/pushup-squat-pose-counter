...
- 맵 비주얼: 보드 12×10으로 확장, ⊓ 경로를 가운데로(양옆 여백 `PATH_MARGIN`, 경로 길이 유지), 설치 구역 = 경로 제외 전체 보드(연한 그리드 패널), 적 경로는 셀 없는 연속 도로(글로우 외곽선 + 센터라인), 디펜스 카메라는 넓어진 보드에 맞춰 후퇴
- 페이즈 전환 배너: 페이즈 변경 시 큰 글자(운동/건설/방어/클리어)가 화면 가운데를 슥 지나가는 트랜지션 (`spawnPhaseBanner`)
...
- **메인 스레드 MediaPipe 성능**: `detectForVideo`(GPU readback 포함)가 프레임 최대 비용. 완화 적용 — 추론을 ~30fps로 스로틀 + 캐릭터가 보일 때만(Build/Defense 일시정지) 실행. 추가 개선은 worker 재도입(WASM worker 초기화 이슈 해결 필요).
- **후처리(블룸) 비용**: 완화 적용 — 픽셀비율 `min(DPR, 1.5)` 캡, 블룸 내부 해상도 절반, 그림자맵 1024, `preserveDrawingBuffer` 제거. 저사양에서 더 필요하면 bloom 비활성 토글 검토.
